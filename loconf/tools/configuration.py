...
def readcvs(args):
    ...
    try:
        ...
        if not args.dont_update:
            config.database.store_cvs(
                args.vehicle, to_be_stored, args.revision_comment)
...
def writecvs(args):
    ...
    if args.all:
        ...
        db_cvs = config.database.get_all_cvs(args.vehicle)
    ...
    if len(write_cvs) == 0:
        ...
        try:
            ...
            if not args.dont_update:
                config.database.store_cvs(args.vehicle, to_be_stored,
...
def readcab(args):
    ...
    vehicles = config.database.query_vehicles(sql.where("address = %i" % cab))
    if len(vehicles) == 0:
